...
- Vitest + React Testing Library (26 test files)
...
├── pages/                     # Feature pages (bathhouses/, bookings/, reviews/, calendar/, chat/, etc.)
├── stores/                    # Zustand: auth.ts (user/token), bathhouse.ts (selected bathhouse)
├── lib/                       # format.ts (kopecks->rubles, day-of-week), useWebSocketNotifications.ts
