...
describe('RecentlyViewed', () => {
  beforeEach(() => {
    vi.mocked(useAuthStore).mockImplementation((selector) => {
      const state = { user: { id: 'u1', role: 'client' }, token: 'tok', setAuth: vi.fn(), setUser: vi.fn(), logout: vi.fn(), isLoading: false, isAuthenticated: true, loadProfile: vi.fn(), notice2faRequiredAt: 0, bumpNotice2FARequired: vi.fn() }
  ...
  it('renders nothing when user is not authenticated', () => {
    vi.mocked(useAuthStore).mockImplementation((selector) => {
      const state = { user: null, token: null, setAuth: vi.fn(), setUser: vi.fn(), logout: vi.fn(), isLoading: false, isAuthenticated: false, loadProfile: vi.fn(), notice2faRequiredAt: 0, bumpNotice2FARequired: vi.fn() }
      return (selector as (s: typeof state) => unknown)(state)
    })
