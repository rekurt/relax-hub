...
vi.mock('@/stores/auth', () => ({
  useAuthStore: vi.fn((selector) =>
    selector({
      user: { id: 'owner-1', name: 'Owner', email: 'owner@test.com', role: 'owner' },
      token: 'test-token',
...
const mockMessages = [
  {
    ...
    sender_id: 'client-111',
    text: 'Здравствуйте, хочу забронировать',
    is_read: true,
  ...
  },
  {
    ...
    conversation_id: 'conv-1',
    sender_id: 'owner-1',
    text: 'Добрый день! Конечно, выбирайте время.',
    is_read: true,
...
describe('ChatPage', () => {
  ...
  it('renders conversation list', () => {
    ...
    renderWithProviders(<ChatPage />)
    expect(screen.getByText('Клиент client-1')).toBeInTheDocument()
    expect(screen.getByText('Клиент client-2')).toBeInTheDocument()
  })

  it('shows messages when conversation is selected (mobile: navigates to message view)', async () => {
    vi.mocked(useGetMyConversations).mockReturnValue({
      data: {
        data: mockConversations,
        success: true,
    ...
    fireEvent.click(screen.getByText('Клиент client-1'))

    await waitFor(() => {
      expect(screen.getByText('Здравствуйте, хочу забронировать')).toBeInTheDocument()
      expect(screen.getByText('Добрый день! Конечно, выбирайте время.')).toBeInTheDocument()
      expect(screen.getByText('Спасибо!')).toBeInTheDocument()
    })
  })

  it('shows back button and message input on mobile after selecting conversation', async () => {
    vi.mocked(useGetMyConversations).mockReturnValue({
    ...
    vi.mocked(useGetConversationsIdMessages).mockReturnValue({
      data: { data: mockMessages, success: true, meta: { total_count: 3 } },
      isLoading: false,
    } as unknown as ReturnType<typeof useGetConversationsIdMessages>)
  ...
  it('returns to conversation list when back button clicked', async () => {
    vi.mocked(useGetMyConversations).mockReturnValue({
      data: { data: mockConversations, success: true, meta: { total_count: 2 } },
      isLoading: false,
    } as unknown as ReturnType<typeof useGetMyConversations>)
    ...
    fireEvent.click(screen.getByText('← Назад к беседам'))
  ...
  it('filters conversations by search', async () => {
    ...
    fireEvent.change(searchInput, { target: { value: 'client-111' } })

    await waitFor(() => {
      expect(screen.getByText('Клиент client-1')).toBeInTheDocument()
      expect(screen.queryByText('Клиент client-2')).not.toBeInTheDocument()
    })
  })
  ...
  it('marks conversation as read when selected with unread messages', async () => {
    ...
    await waitFor(() => {
      expect(markReadFn).toHaveBeenCalledWith({ id: 'conv-1' })
    })
  })
})
