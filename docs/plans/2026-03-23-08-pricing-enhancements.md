...
- [x] Long session discount (FR-084):
...
- [x] Extra guest surcharge (FR-085):
...
- [x] Migration: add `long_session_threshold_hours INT DEFAULT 4`, `long_session_discount_percent INT DEFAULT 0`, `base_capacity INT`, `extra_guest_surcharge BIGINT DEFAULT 0` to bathhouses
- [x] Modify price calculation flow to include these rules (after base price, before discounts)
- [x] Include breakdown in price response:
  ```json
  {
    "base_price": 1000000,
  ...
  ```
- [x] Write tests with various combinations
...
- [x] Holiday model: ID, Name (string), Date (time.Time), Region (string: "RU"/"BY"), IsRecurring (bool — same date every year)
- [x] BathhouseHolidayPrice model: BathhouseID, Multiplier (float64, range 1.0-2.0, step 0.1)
  - Per-bathhouse multiplier applied on holidays
  - If not set, use platform default (1.5x)
- [x] HolidayRepository: Create, Update, Delete, ListByRegion, GetByDate, ListAll
- [x] Admin endpoints:
...
- [x] Integrate into price calculation:
...
- [x] Seed initial holidays: New Year (1-8 Jan), 23 Feb, 8 Mar, 1 May, 9 May, 12 Jun, 4 Nov
- [x] Write tests
- [x] Run `go test ./... -v` — must pass
...
- [x] Migration: add these columns to bathhouses
- [x] Logic: when an unbooked slot starts within LastMinuteHoursThreshold hours from now:
...
- [x] Add last_minute filter to search:
  - GET /api/v1/bathhouses?last_minute=true — show only bathhouses with active last-minute deals
- [x] Include "last_minute" badge in bathhouse list response when applicable
- [x] Write tests
- [x] Run `go test ./... -v` — must pass
...
- [x] Migration: service_fee_configs table
- [x] ServiceFeeService:
...
- [x] Add ServiceFeeAmount (int64) to Booking model
- [x] Migration: add `service_fee_amount BIGINT DEFAULT 0` to bookings
- [x] Write tests
