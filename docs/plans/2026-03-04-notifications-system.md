...
- [x] WebSocket endpoint: GET /api/v1/ws/notifications — подключение по JWT-токену
- [x] Hub-паттерн: регистрация/отключение клиентов, отправка уведомлений подключенным пользователям
- [x] При создании in-app уведомления: отправить через WebSocket если пользователь онлайн
- [x] Heartbeat/ping-pong для поддержания соединения
- [x] Написать тесты
- [x] Запустить go test ./... - все тесты должны пройти
