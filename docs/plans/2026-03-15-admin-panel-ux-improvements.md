...
- [ ] Заменить `location.reload()` после approve/reject на DOM-манипуляцию:
...
- [ ] Реализовать toast-уведомления вместо alert():
...
- [ ] Добавить loading state на кнопки действий:
...
- [ ] Добавить confirmation dialog для batch approve (сейчас есть только для reject):
    "Одобрить N отзывов? Это действие нельзя отменить."
- [ ] Динамически обновлять stats-карточки после каждого действия:
    pending -1, approved/rejected +1, без перезагрузки
- [ ] Для batch операций: удалять обработанные карточки из DOM по одной с анимацией,
    показывать toast "Обработано X из Y отзывов" (учитывая что часть могла быть
    уже обработана другим админом)
- [ ] Добавить поле free-text в reject modal: textarea "Дополнительный комментарий"
    (необязательное) помимо чекбоксов с причинами
- [ ] Добавить валидацию: при отклонении должна быть выбрана хотя бы одна причина
    или заполнен комментарий (сейчас можно отклонить без причины)
- [ ] Добавить lightbox для изображений отзывов: клик по 80x80 thumbnail открывает
    полноразмерное изображение в overlay с кнопкой закрытия и навигацией стрелками
- [ ] Добавить опцию "Скрытые" в фильтр по статусу (CSS стили для hidden уже есть,
    но в select нет такой опции)
- [ ] Добавить усечение длинного текста отзыва: показывать первые 200 символов
    с кнопкой "Показать полностью" (expand/collapse)
- [ ] Write tests: проверить JSON-ответы API для inline обновлений, валидацию причин
...
- [ ] Сделать KPI-карточки кликабельными:
...
- [ ] Добавить индикаторы трендов на KPI-карточки:
...
- [ ] Улучшить карточку выручки: разделить Today/Week/Month на 3 отдельные sub-cards
    вместо одной строки через "/" (сейчас трудно читать)
- [ ] Добавить относительное время в ленте активности: "5 мин назад", "2 часа назад"
    (Go template function `relativeTime` + JS для обновления без reload)
- [ ] Добавить preview текста отзыва в ленте последних отзывов (первые 100 символов)
- [ ] Добавить empty-state для каждой ленты: вместо "Нет данных" показывать
    иконку + текст "За выбранный период нет бронирований"
- [ ] Добавить toggle автообновления: кнопка play/pause, интервал 60 сек, по умолчанию
    выключено, рядом текст "Обновлено: HH:MM:SS"
- [ ] Применить `text-truncate` к именам пользователей (сейчас только на банях)
- [ ] Добавить быструю ссылку "Посмотреть все" внизу каждой ленты активности
    (→ на соответствующий раздел GoAdmin)
- [ ] Write tests: проверить расчет трендов, форматирование relative time,
