...
- [ ] Migration: add `booking_mode VARCHAR(10) DEFAULT 'instant'`, `request_timeout INT DEFAULT 24` to bathhouses; add `hold_id UUID` to bookings
- [ ] Modify CreateBooking in service:
...
- [ ] Cron: auto-reject requests older than RequestTimeout hours
  - Release hold, set status rejected
  - Notify both parties
- [ ] Write tests for both instant and request flows
...
- [ ] Add to Booking model: CheckedInAt (*time.Time), CheckedOutAt (*time.Time)
- [ ] New booking status: "no_show"
- [ ] Migration: add `checked_in_at TIMESTAMPTZ`, `checked_out_at TIMESTAMPTZ` to bookings
- [ ] PATCH /api/v1/bookings/{id}/check-in — owner marks guest arrival (RequireAuth, owner/rep)
  - Validate: booking is confirmed, current time is within [start - 15min, start + 30min]
  - Set CheckedInAt = now
- [ ] PATCH /api/v1/bookings/{id}/check-out — owner marks guest departure (RequireAuth, owner/rep)
  - Validate: booking is checked in
  - Set CheckedOutAt = now, status = completed
- [ ] Owner reminder: push notification 5 min before session start ("Гость скоро прибудет!")
- [ ] No-show detection cron (every 15 min):
...
- [ ] No-show dispute: client can dispute within 2 hours
...
- [ ] Reminder cron job (every 15 min):
...
- [ ] POST /api/v1/bookings/{id}/extend — extend booking by N hours (RequireAuth)
  ...
  - Notify both parties
- [ ] Return updated booking with new end time and extension payment details
- [ ] Write tests
- [ ] Run `go test ./... -v` — must pass
...
- [ ] GET /api/v1/bookings/{id}/rebook-data — return pre-filled booking data from past booking (RequireAuth)
...
- [ ] Validate bathhouse is still active
- [ ] Write tests
- [ ] Run `go test ./... -v` — must pass
...
- [ ] Migration: add these columns to bathhouses with defaults
- [ ] Modify GetAvailableSlots logic:
...
- [ ] Owner settings endpoints (PUT /api/v1/my/bathhouses/{id}) — add these fields to update
