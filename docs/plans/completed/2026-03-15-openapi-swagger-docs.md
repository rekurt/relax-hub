...
- [x] Запустить `swag init` — полная генерация без ошибок и ворнингов
- [x] Запустить `go build ./...` — компиляция без ошибок
- [x] Запустить `make lint` — без новых ошибок линтера
- [x] Проверить swagger.json — все эндпоинты из router.go присутствуют в спеке
- [x] Проверить Swagger UI — корректно рендерится, Try It Out работает с Bearer токеном
