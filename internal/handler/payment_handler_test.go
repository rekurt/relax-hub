...
func (m *mockPaymentService) RefundPayment(ctx context.Context, bookingID uuid.UUID, forceFullRefund bool, refundTo string, policy domain.CancellationPolicy) error {
	if m.refundFn != nil {
...
func TestPaymentHandler_InitiatePayment(t *testing.T) {
	...
	svc := &mockPaymentService{
		initiateFn: func(ctx context.Context, uID, bID uuid.UUID, method domain.PaymentMethod) (string, error) {
			if bID != bookingID {
