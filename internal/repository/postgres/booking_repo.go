...
func (r *bookingRepo) Create(ctx context.Context, booking *domain.Booking) error {
	query := `
		INSERT INTO bookings (id, user_id, bathhouse_id, start_time, end_time, guest_count, total_price, addon_total, service_fee_amount, points_spent, referral_bonus_used, status, comment, created_at, updated_at)
	...
	_, err := r.pool.Exec(ctx, query,
		...
		booking.TotalPrice, booking.AddOnTotal, booking.ServiceFeeAmount, booking.PointsSpent, booking.ReferralBonusUsed, booking.Status, booking.Comment,
...
func (r *bookingRepo) GetByID(ctx context.Context, id uuid.UUID) (*domain.Booking, error) {
	query := `
		SELECT id, user_id, bathhouse_id, start_time, end_time, guest_count, total_price, addon_total, service_fee_amount, points_spent, referral_bonus_used, status, comment, created_at, updated_at
	...
	err := r.pool.QueryRow(ctx, query, id).Scan(
		...
		&b.TotalPrice, &b.AddOnTotal, &b.ServiceFeeAmount, &b.PointsSpent, &b.ReferralBonusUsed, &b.Status, &b.Comment,
