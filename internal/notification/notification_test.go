...
func TestDispatcher_Dispatch_AllChannelsEnabled(t *testing.T) {
	...
	d := notification.NewDispatcher(notifRepo, nil, emailSender, nil, mock.NewTelegramLinkRepo(), nil, nil, nil, hub, log)
...
func TestDispatcher_Dispatch_TelegramEnabled(t *testing.T) {
	...
	log := logger.New(logger.LevelError)
	hub := notification.NewHub(log)
	d := notification.NewDispatcher(notifRepo, nil, emailSender, tgSender, tgRepo, nil, nil, nil, hub, log)
